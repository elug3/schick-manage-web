                                    ...
                                    : "text-[#6B6480] hover:bg-[#F4F3F8] hover:text-[#1C1B1F]",
                                        ...
                                        activeTab === tab.value
                                            ...
                                            : "bg-[#F4F3F8] text-[#6B6480]",
                                                ...
                                                "Date",
                                                "",
                                            ].map((h) => (_jsx("th", { className: "px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-[#9D98B3]", children: h }, h))) }) }), _jsx("tbody", { children: filtered.map((order) => (_jsx(OrderRows, { order: order, skuLookup: skuLookup, expanded: expandedId === order.id, updating: updatingId === order.id, onToggle: () => setExpandedId(expandedId === order.id ? null : order.id), onAction: (action) => handleOrderAction(order, action) }, order.id))) })] }) })] })) })] }));
}
