...
- Users carry `permissions: string[]` and `account_type: "customer" | "manager" | "service"` (see `PERMISSION_CATALOG` / `AccountType` in `app/lib/api.ts`) — includes `product.master.read|write`. The legacy `roles` claim was removed from the backend. Auth still wires human operators as `admin` on the wire; manage-web normalizes that to `manager` (admin is a permission/management tier).
