...
- Users carry `permissions: string[]` and `account_type: "customer" | "manager" | "service"` (see `PERMISSION_CATALOG` / `AccountType` in `app/lib/api.ts`) — includes `product.master.read|write`. The legacy `roles` claim was removed from the backend. Auth may still store human operators as wire value `admin`; manage-web normalizes that to `manager` (admin is a permission/management tier). Backend rename instruct: [docs/ai-instruct-dupli1-auth-account-type-manager.md](docs/ai-instruct-dupli1-auth-account-type-manager.md).
